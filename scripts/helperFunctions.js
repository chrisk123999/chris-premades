...
export let chris = {
    ...
    'createEffect': async function _createEffect(actor, effectData, concentrationItem) {
        ...
        if (concentrationItem) {
            await chris.addDependent(MidiQOL.getConcentrationEffect(concentrationItem.actor, concentrationItem), [effectToReturn]);
        }
        return effectToReturn;
    },
    'addDependent': async function _addDependent(parentDocument, dependents) {
        await parentDocument?.addDependent(...dependents);
    },
                ...
                content +=
                    `<div class = 'form-group'>
                        ...
                        <label>
                            ...
                            <p style='padding: 1%; text-align: center; font-size: 15px;'` + (displayTooltips ? ` data-tooltip="${i.system.description.value.replace(/<[^>]*>?/gm, '')}"` : ``) + `>` + documents[i].name + (documents[i].system?.details?.cr != undefined ? ` (CR ${chris.decimalToFraction(documents[i].system?.details?.cr)})` : ``) + `</p>
                        </label>
                    </div>
                `;
            }
