...
async function use({workflow}) {
    ...
    if (creatureType === 'earth') {
        genericUtils.setProperty(updates, 'actor.system.attributes.movement.burrow', genericUtils.convertDistance(40));
        ...
        if (creatureType === 'air') {
            genericUtils.setProperty(updates, 'actor.system.attributes.movement', {fly: genericUtils.convertDistance(40), hover: true});
            genericUtils.setProperty(updates, 'actor.system.traits.dr.value', ['lightning', 'thunder']);
        } else if (creatureType === 'water') {
            genericUtils.setProperty(updates, 'actor.system.attributes.movement.swim', genericUtils.convertDistance(40));
