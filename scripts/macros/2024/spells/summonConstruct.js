...
async function use({workflow}) {
    ...
    let sourceActor = await compendiumUtils.getActorFromCompendium(constants.modernPacks.summons, 'CPR - Construct Spirit');
    if (!sourceActor) {
    ...
    let multiAttackFeatureData = await Summons.getSummonItem('Multiattack (Construct Spirit)', {}, workflow.item, {translate: genericUtils.format('CHRISPREMADES.CommonFeatures.Multiattack', {numAttacks}), identifier: 'summonConstructMultiattack', rules: 'modern'});
    let slamFeatureData = await Summons.getSummonItem('Slam (Construct Spirit)', {}, workflow.item, {translate: 'CHRISPREMADES.Macros.SummonConstruct.Slam', identifier: 'summonConstructSlam', flatAttack: true, damageBonus: spellLevel, rules: 'modern'});
    ...
    if (creatureType === 'clay') {
        let berserkLashingData = await Summons.getSummonItem('Berserk Lashing (Clay Only)', {}, workflow.item, {translate: 'CHRISPREMADES.Macros.SummonConstruct.BerserkLashing', identifier: 'summonConstructBerserkLashing', rules: 'modern'});
        ...
        let heatedBodyData = await Summons.getSummonItem('Heated Body (Metal Only)', {}, workflow.item, {translate: 'CHRISPREMADES.CommonFeatures.HeatedBody', identifier: 'summonConstructHeatedBody', rules: 'modern'});
        if (!heatedBodyData) {
            errors.missingPackItem();
            if (concentrationEffect) await genericUtils.remove(concentrationEffect);
            return;
        ...
        let stoneLethargyData = await Summons.getSummonItem('Stone Lethargy (Stone Only)', {}, workflow.item, {flatDC: true, translate: 'CHRISPREMADES.CommonFeatures.StoneLethargy', identifier: 'summonConstructStoneLethargy', rules: 'modern'});
...
async function turnStartStone({trigger: {entity: item, token, target}}) {
...
export let summonConstructMetal = {
    name: 'Summon Construct: Metal',
    version: summonConstruct.version,
    rules: summonConstruct.rules,
    combat: [
        {
            pass: 'turnStartNear',
            macro: turnStartMetal,
            distance: 5,
            priority: 50
        }
    ]
