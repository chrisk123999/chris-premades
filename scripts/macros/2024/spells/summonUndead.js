...
async function use({workflow}) {
    ...
    let sourceActor = await compendiumUtils.getActorFromCompendium(constants.modernPacks.summons, 'CPR - Undead Spirit');
    if (!sourceActor) {
    ...
    let multiAttackFeatureData = await Summons.getSummonItem('Multiattack (Undead Spirit)', {}, workflow.item, {translate: genericUtils.format('CHRISPREMADES.CommonFeatures.Multiattack', {numAttacks}), identifier: 'summonUndeadMultiattack', rules: 'modern'});
    ...
    if (creatureType === 'ghostly') {
        hpFormula += 30;
        let incorporealPassageData = await Summons.getSummonItem('Incorporeal Passage (Ghostly Only)', {}, workflow.item, {translate: 'CHRISPREMADES.Macros.SummonUndead.IncorporealPassage', identifier: 'summonUndeadIncorporealPassage', rules: 'modern'});
        let deathlyTouchData = await Summons.getSummonItem('Deathly Touch (Ghostly Only)', {}, workflow.item, {flatAttack: true, damageBonus: spellLevel, translate: 'CHRISPREMADES.Macros.SummonUndead.DeathlyTouch', identifier: 'summonUndeadDeathlyTouch', rules: 'modern'});
        ...
        hpFormula += 20;
        let graveBoltData = await Summons.getSummonItem('Grave Bolt (Skeletal Only)', {}, workflow.item, {flatAttack: true, damageBonus: spellLevel, translate: 'CHRISPREMADES.Macros.SummonUndead.GraveBolt', identifier: 'summonUndeadGraveBolt', rules: 'modern'});
        if (!graveBoltData) {
            errors.missingPackItem();
            if (concentrationEffect) await genericUtils.remove(concentrationEffect);
            return;
        ...
        hpFormula += 30;
        let festeringAuraData = await Summons.getSummonItem('Festering Aura (Putrid Only)', {}, workflow.item, {flatDC: true, translate: 'CHRISPREMADES.Macros.SummonUndead.FesteringAura', identifier: 'summonUndeadFesteringAura', rules: 'modern'});
        let rottingClawData = await Summons.getSummonItem('Rotting Claw (Putrid Only)', {}, workflow.item, {flatAttack: true, damageBonus: spellLevel, translate: 'CHRISPREMADES.Macros.SummonUndead.RottingClaw', identifier: 'summonUndeadRottingClaw', rules: 'modern'});
        let rottingClawParalyzeData = await Summons.getSummonItem('Rotting Claw (Putrid Only): Paralyze', {}, workflow.item, {translate: 'CHRISPREMADES.Macros.SummonUndead.RottingClawParalyze', identifier: 'summonUndeadRottingClawParalyze', rules: 'modern'});
