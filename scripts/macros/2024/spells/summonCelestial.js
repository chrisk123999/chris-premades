...
async function use({workflow}) {
    ...
    let sourceActor = await compendiumUtils.getActorFromCompendium(constants.modernPacks.summons, 'CPR - Celestial Spirit');
    if (!sourceActor) {
    ...
    let multiAttackFeatureData = await Summons.getSummonItem('Multiattack (Celestial Spirit)', {}, workflow.item, {translate: genericUtils.format('CHRISPREMADES.CommonFeatures.Multiattack', {numAttacks}), identifier: 'summonCelestialMultiattack', rules: 'modern'});
    let healingTouchFeatureData = await Summons.getSummonItem('Healing Touch (Celestial Spirit)', {}, workflow.item, {translate: 'CHRISPREMADES.Macros.SummonCelestial.HealingTouch', identifier: 'summonCelestialHealingTouch', damageBonus: spellLevel, rules: 'modern'});
    ...
    if (creatureType === 'avenger') {
        let radiantBowData = await Summons.getSummonItem('Radiant Bow (Avenger Only)', {}, workflow.item, {flatAttack: true, damageBonus: spellLevel, translate: 'CHRISPREMADES.Macros.SummonCelestial.RadiantBow', identifier: 'summonCelestialRadiantBow', rules: 'modern'});
        if (!radiantBowData) {
            errors.missingPackItem();
            if (concentrationEffect) await genericUtils.remove(concentrationEffect);
            return;
        ...
        let radiantMaceData = await Summons.getSummonItem('Radiant Mace (Defender Only)', {}, workflow.item, {flatAttack: true, damageBonus: spellLevel, translate: 'CHRISPREMADES.Macros.SummonCelestial.RadiantMace', identifier: 'summonCelestialRadiantMace', rules: 'modern'});
        if (!radiantMaceData) {
            errors.missingPackItem();
            if (concentrationEffect) await genericUtils.remove(concentrationEffect);
            return;
