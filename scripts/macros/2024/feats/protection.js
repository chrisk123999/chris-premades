...
async function early({trigger: {entity: item, token}, workflow}) {
    ...
    if (tokenUtils.getDistance(token, targetToken, {wallsBlock: true}) > genericUtils.handleMetric(5)) return;
...
async function moved({trigger: {token, entity: effect}, options}) {
    ...
    if (targetEffect)
    {
        ...
        if (!protector) return;
        if (tokenUtils.getDistance(token, protector, {wallsBlock: true}) > genericUtils.handleMetric(5))
        {
    ...
    else if (protectorEffect)
    {
        ...
        if (!target) return;
        if (tokenUtils.getDistance(token, target, {wallsBlock: true}) > genericUtils.handleMetric(5))
        {
