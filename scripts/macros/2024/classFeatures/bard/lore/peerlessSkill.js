...
async function attackEarly({trigger: {entity: item}, workflow}) {
    if (!workflow.targets.size || workflow.isFumble || !constants.attacks.includes(workflowUtils.getActionType(workflow))) return;
...
async function attackLate({trigger, workflow}) {
    if (!workflow?.['chris-premades']?.peerlessSkill || !workflow.hitTargets.size || !constants.attacks.includes(workflowUtils.getActionType(workflow))) return;
    let bardicInspiration = itemUtils.getItemByIdentifier(workflow.actor, 'bardicInspiration');
