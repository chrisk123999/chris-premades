...
async function early({trigger: {entity: item}, workflow}) {
    ...
    if (!workflow.token || !workflow.targets.size || workflow.disadvantage || (workflow.disadvantage && workflow.advantage) || !combatUtils.isOwnTurn(workflow.token) || !constants.attacks.includes(workflowUtils.getActionType(workflow))) return;
