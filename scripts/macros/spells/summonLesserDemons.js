...
async function use({workflow}) {
    ...
    let compendiumDocs = await compendiumUtils.getFilteredDocumentsFromCompendium(monsterCompendium, {maxCR, actorTypes: ['npc'], creatureSubtypes: ['demon']});
    ...
    let sourceDocs = await dialogUtils.selectDocumentsDialog(workflow.item.name, genericUtils.format('CHRISPREMADES.Summons.SelectSummons', {totalSummons: summonsMultiplier * 2 / maxCR}), compendiumDocs, {max: summonsMultiplier * 2 / maxCR, sortAlphabetical: true});
