...
async function use({workflow}) {
    ...
    let compendiumDocs = await compendiumUtils.getFilteredDocumentsFromCompendium(monsterCompendium, {maxCR: spellLevel, actorTypes: ['npc'], creatureTypes: ['elemental']});
    ...
    let sourceActor = await dialogUtils.selectDocumentDialog(workflow.item.name, 'CHRISPREMADES.Summons.SelectSummon', compendiumDocs, {sortAlphabetical: true, userId});
    if (!sourceActor) {
        if (concentrationEffect) await genericUtils.remove(concentrationEffect);
        return;
    }
