...
export async function summonFiend({speaker, actor, token, character, item, args}){
    ...
    switch (selection) {
        case 'Demon':
            hpFormula = 50 + ((this.castData.castLevel - 6) * 15);
            ...
            deathThroesData.system.save.dc = chris.getSpellDC(this.item);
            ...
            biteData.system.damage.parts[0][0] += ' + ' + this.castData.castLevel;
            updates.embedded.Item[biteData.name] = biteData;
            break;
        case 'Devil':
            hpFormula = 40 + ((this.castData.castLevel - 6) * 15);
            updates.actor.system.attributes.hp = {
                'formula': hpFormula,
                'max': hpFormula,
                'value': hpFormula
            ...
            hurlFlameData.system.damage.parts[0][0] += ' + ' + this.castData.castLevel;
            ...
            break;
        case 'Yugoloth':
            hpFormula = 60 + ((this.castData.castLevel - 6) * 15);
            updates.actor.system.attributes.hp = {
                'formula': hpFormula,
                'max': hpFormula,
                'value': hpFormula
