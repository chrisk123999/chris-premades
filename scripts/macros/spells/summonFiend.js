...
async function use({workflow}) {
    ...
    let multiAttackFeatureData = await Summons.getSummonItem('Multiattack (Fiendish Spirit)', {}, workflow.item, {translate: genericUtils.format('CHRISPREMADES.CommonFeatures.Multiattack', {numAttacks}), identifier: 'summonFiendMultiattack'});
    let magicResistanceFeatureData = await Summons.getSummonItem('Magic Resistance (Fiendish Spirit)', {}, workflow.item, {translate: 'CHRISPREMADES.CommonFeatures.MagicResistance', identifier: 'magicResistance'});
    ...
    if (creatureType === 'demon') {
        hpFormula += 50;
        let deathThroesData = await Summons.getSummonItem('Death Throes (Demon Only)', {}, workflow.item, {translate: 'CHRISPREMADES.Macros.SummonFiend.DeathThroes', identifier: 'summonFiendDeathThroes', flatDC: true, damageBonus: spellLevel});
        let biteData = await Summons.getSummonItem('Bite (Demon Only)', {}, workflow.item, {translate: 'CHRISPREMADES.CommonFeatures.Bite', identifier: 'summonFiendBite', flatAttack: true, damageBonus: spellLevel});
        if (!deathThroesData || !biteData) {
            errors.missingPackItem();
            if (concentrationEffect) await genericUtils.remove(concentrationEffect);
            return;
        }
        genericUtils.setProperty(updates, 'actor.system.attributes.movement.climb', 40);
        updates.actor.items.push(biteData, deathThroesData);
    } else if (creatureType === 'devil') {
        hpFormula += 40;
        let hurlFlameData = await Summons.getSummonItem('Hurl Flame (Devil Only)', {}, workflow.item, {flatAttack: true, damageBonus: spellLevel, translate: 'CHRISPREMADES.Macros.SummonFiend.HurlFlame', identifier: 'summonFiendHurlFlame'});
        let devilsSightData = await Summons.getSummonItem('Devil\'s Sight (Devil Only)', {}, workflow.item, {translate: 'CHRISPREMADES.CommonFeatures.DevilsSight', identifier: 'devilsSight'});
        if (!hurlFlameData || !devilsSightData) {
            errors.missingPackItem();
            if (concentrationEffect) await genericUtils.remove(concentrationEffect);
            return;
        }
        genericUtils.setProperty(updates, 'actor.system.attributes.movement.fly', 60);
        updates.actor.items.push(hurlFlameData, devilsSightData);
    } else {
        hpFormula += 60;
        let clawsData = await Summons.getSummonItem('Claws (Yugoloth Only)', {}, workflow.item, {flatAttack: true, damageBonus: spellLevel, translate: 'CHRISPREMADES.CommonFeatures.Claws', identifier: 'summonFiendClaws'});
        if (!clawsData) {
            errors.missingPackItem();
            if (concentrationEffect) await genericUtils.remove(concentrationEffect);
            return;
