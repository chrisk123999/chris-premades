...
async function use({workflow}) {
    ...
    if (investmentOfTheChainMaster) {
        ...
        for (let i = 0; i < updates.length; i++) {
            ...
            genericUtils.setProperty(updates[i], 'actor.system.attributes.movement.' + movement, genericUtils.convertDistance(40));
        }
        if (!findFamiliarEffect) {
...
async function late({workflow}) {
    ...
    for (let i of familiarTokens) {
        if (tokenUtils.getDistance(workflow.token, i) > genericUtils.convertDistance(100)) familiarTokens.delete(i);
    }
...
async function early({workflow}) {
    ...
    if (!familiarTokens?.size) return;
    for (let i of familiarTokens) {
        if (tokenUtils.getDistance(workflow.targets.first(), i) > genericUtils.convertDistance(5)) familiarTokens.delete(i);
    }
    if (!familiarTokens.size) return;
