...
async function use({workflow}) {
    ...
    let attackData = await compendiumUtils.getItemFromCompendium(constants.featurePacks.summonFeatures, 'Slam (Animated Object)', {object: true, getDescription: true, translate: 'CHRISPREMADES.Macros.AnimateObjects.Attack', flatAttack: 6});
    ...
    choices.forEach(c => {
        ...
        if (d.scale) {
            genericUtils.setProperty(update, 'token.texture.scaleX', d.scale);
            genericUtils.setProperty(update, 'token.texture.scaleY', d.scale);
        }
        genericUtils.setProperty(update, 'actor.system.traits.size', d.id);
        let data = getSizeData(d.id);
        genericUtils.setProperty(update, 'actor.system.attributes.hp', data.hp);
        genericUtils.setProperty(update, 'actor.system.attributes.ac', data.ac);
