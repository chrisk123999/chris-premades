...
async function early({trigger, workflow}) {
    ...
    if (shape === 'circle') {
        ...
        let lengthButtons = Array.from(Array(12).keys().map(i => genericUtils.convertDistance(5) * (i + 1))).toSorted((a, b) => b - a).map(i => [genericUtils.format('CHRISPREMADES.Distance.DistanceFeet', {distance: i}), i]);
