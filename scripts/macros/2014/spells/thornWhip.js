...
async function use({trigger, workflow}) {
    ...
    for (let target of workflow.hitTargets) {
        ...
        if (distance <= genericUtils.convertDistance(5)) break;
        if (distance > genericUtils.convertDistance(5)) options.push([genericUtils.format('CHRISPREMADES.Distance.DistanceFeet', {distance: 5}), 5]);
        if (distance > genericUtils.convertDistance(10)) options.push([genericUtils.format('CHRISPREMADES.Distance.DistanceFeet', {distance: 10}), 10]);
