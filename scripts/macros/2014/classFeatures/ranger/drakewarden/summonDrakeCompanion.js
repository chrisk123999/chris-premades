...
async function use({workflow}) {
    ...
    switch (drakeUpgrades) {
        case 1:
            genericUtils.setProperty(updates, 'actor.system.attributes.movement.fly', genericUtils.convertDistance(40));
...
async function postAttack({trigger: {entity: effect, token}, workflow}) {
    ...
    if (tokenUtils.getDistance(token, summonToken) > genericUtils.convertDistance(30)) return;
    let selection;
...
async function postDamage({trigger: {entity: item, token}, workflow}) {
    ...
    if (tokenUtils.getDistance(token, workflow.token) > genericUtils.convertDistance(30)) return;
