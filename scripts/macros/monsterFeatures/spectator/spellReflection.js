...
export async function spellReflection({speaker, actor, token, character, item, args, scope, workflow}) {
    if (args[0].macroPass === 'isSaveSuccess' && workflow.item.type === 'spell') {
        const oldToken = args[0].options.token;
        ...
        let originItem = chris.getItem(args[0].options.actor, "Spell Reflection");
      ...
      if (args[0].macroPass === 'isAttacked' && workflow.item.type === 'spell') {
        ...
        let originItem = chris.getItem(args[0].options.actor, "Spell Reflection");
        const msg = await chris.getItem(originItem, "Spell Reflection").displayCard({createMessage:false});
        const DIV = document.createElement("DIV");
        DIV.innerHTML = msg.content;
        DIV.querySelector("div.card-buttons").remove();
        await ChatMessage.create({content:DIV.innerHTML});
