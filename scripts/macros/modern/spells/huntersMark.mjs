...
function getDamageConfig(actor, document) {
    ...
    const source = foeSlayer && automationUtils.getConfigValue(foeSlayer, 'formula') ? foeSlayer : document;
    return automationUtils.getConfigValues(source, ['formula', 'damageType']);
}
function markedEffectData(activity, document, seconds) {
    return documentUtils.getBaseEffectData(activity, {
        name: _loc('CHRISPREMADES.Macros.Modern.HuntersMark.Marked'),
        img: document.img,
...
async function use({document, workflow}) {
    ...
    const casterEffectData = documentUtils.getBaseEffectData(workflow.activity, {
        ...
        macros: [{type: 'roll', macros: [{source: 'chris-premades', rules: '2024', identifier: 'hunters-mark-attack'}]}]
    });
    genericUtils.setProperty(casterEffectData, 'flags.chris-premades.huntersMark', {
        ...
        ...getDamageConfig(workflow.actor, document)
    });
...
async function move({document, workflow}) {
    ...
    if (current.length > 1) {
        const selection = await dialogUtils.selectTargetDialog(document.name, _loc('CHRISPREMADES.Macros.Modern.HuntersMark.Multiple'), current, {skipDeadAndUnconscious: false});
    ...
    if (!isValidTarget(newTarget, markData.targets)) newTarget = await selectNewTarget(document, workflow.token.document, workflow.activity.range.value, markData.targets, 'CHRISPREMADES.Macros.Modern.HuntersMark');
    if (!newTarget) return;
...
async function promptMove({document, token}) {
    ...
    if (!await dialogUtils.confirm(document.name, _loc('CHRISPREMADES.Macros.Modern.HuntersMark.MovePrompt'), {userId})) return;
    const newTarget = await selectNewTarget(document, token, activity.range.value, markedUuids, 'CHRISPREMADES.Macros.Modern.HuntersMark', {userId});
    if (!newTarget) return;
...
async function damage({document, workflow}) {
    ...
    const bonus = new DamageBonus(document, {formula: markData.formula, optional: false, maxTargets: 1, type: markData.damageType});
