...
async function use({workflow}) {
    ...
    if (playAnimation) genericUtils.setProperty(teleportData, 'flags.chris-premades.config.playAnimation', playAnimation);
    ...
    let vaeButtons = [
        {type: 'use', name: teleportData.name, identifier: 'manifestEchoTeleport'},
        {type: 'use', name: attackData.name, identifier: 'manifestEchoAttack'}
    ];
    let echoAvatar = itemUtils.getItemByIdentifier(workflow.actor, 'echoAvatar');
    if (echoAvatar) vaeButtons.push({type: 'use', name: teleportData.name, identifier: 'echoAvatar'});
...
export let manifestEchoTeleport = {
...
export let manifestEchoAttack = {
    name: 'Manifest Echo: Attack',
    version: manifestEcho.version,
    midi: {
        item: [
            {
