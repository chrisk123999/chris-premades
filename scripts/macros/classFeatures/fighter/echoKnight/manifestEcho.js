...
async function item({
  ...
  let sceneEchos = canvas.scene.tokens.filter(
    (i) =>
      i.actor?.flags?.["chris-premades"]?.feature?.manifestEcho?.ownerUuid ===
      workflow.actor.uuid,
  );
  ...
  if (tokenImg && tokenImg != "") {
    ...
    setProperty(
      updates,
      "token.texture.src",
      workflow.token.document.texture.src,
    );
    setProperty(
      updates,
  ...
  let animationName =
    chris.getConfiguration(workflow.item, "animation") ?? "default";
  if (chris.jb2aCheck() != "patreon" || !chris.aseCheck()) {
    animationName = "none";
  }
  let spawnedTokens = await chris.spawn(
    sourceActor,
    updates,
    {},
    workflow.token,
  ...
  let effect = workflow.actor.effects.find(
    (i) => i.flags["chris-premades"]?.feature?.manifestEcho,
  );
  if (effect) return;
  let mutationStack = warpgate.mutationStack(workflow.token.document);
  if (mutationStack.getName("Manifest Echo")) {
    await warpgate.revert(workflow.token.document, "Manifest Echo");
  }
  let featureData = await chris.getItemFromCompendium(
    "chris-premades.CPR Class Feature Items",
    "Manifest Echo - Dismiss",
    false,
  );
  if (!featureData) return;
  featureData.system.description.value = chris.getItemDescription(
    "CPR - Descriptions",
    "Manifest Echo - Dismiss",
  );
  let featureData2 = await chris.getItemFromCompendium(
    "chris-premades.CPR Class Feature Items",
    "Manifest Echo - Teleport",
    false,
  );
  if (!featureData2) return;
  featureData2.system.description.value = chris.getItemDescription(
    "CPR - Descriptions",
    "Manifest Echo - Teleport",
  );
  let featureData3 = await chris.getItemFromCompendium(
    "chris-premades.CPR Class Feature Items",
    "Manifest Echo - Attack",
    false,
  );
  if (!featureData3) return;
  featureData3.system.description.value = chris.getItemDescription(
    "CPR - Descriptions",
    "Manifest Echo - Attack",
  );

  async function effectMacro() {
    await chrisPremades.macros.manifestEcho.dismiss({
      workflow: { actor, token },
    });
  }
...
async function teleport({
  ...
  if (sceneEchos.length > 1) {
    let selection = await chris.selectTarget(
      workflow.item.name,
      constants.okCancel,
      sceneEchos.map((i) => i.object),
      true,
      "one",
      null,
      false,
...
async function attack({
  ...
  let features = workflow.actor.items.filter(
    (i) =>
      constants.weaponAttacks.includes(i.system.actionType) &&
...
async function turnEnd(token) {
  ...
  for (let i of sceneEchos) {
    ...
    if (distance > maxRange) {
      let selection = await chris.remoteDialog(
        "Echo Knight",
        constants.yesNo,
        chris.lastGM(),
        token.actor.name + "'s echo has moved far away. Remove it?",
      );
...
async function save(actor, roll, ability) {
  let ownerUuid =
    actor.flags["chris-premades"]?.feature?.manifestEcho?.ownerUuid;
  if (!ownerUuid) return;
  ...
  let newRoll = new Roll("0 + " + formula, owner.getRollData()).evaluate({
    async: false,
  });
  for (let i = 1; i < newRoll.terms.length; i++)
    roll.terms.push(newRoll.terms[i]);
...
async function defeated(token, effect) {
  let sceneEchos = canvas.scene.tokens.filter(
    (i) =>
      i.actor?.flags?.["chris-premades"]?.feature?.manifestEcho?.ownerUuid ===
        token.actor.flags["chris-premades"].feature.manifestEcho.ownerUuid &&
      i.id != token.id,
  );
  if (!sceneEchos.length) {
    let ownerTokenUuid =
      effect.flags["chris-premades"]?.feature?.manifestEcho?.ownerTokenUuid;
    if (ownerTokenUuid) {
      let ownerToken = await fromUuid(ownerTokenUuid);
      if (ownerToken) {
        let effect2 = ownerToken.actor.effects.find(
          (i) => i.flags["chris-premades"]?.feature?.manifestEcho,
        );
