...
async function use({workflow}) {
    let featureData = await compendiumUtils.getItemFromCompendium(constants.featurePacks.classFeatureItems, 'Dread Lord: Shadow Attack', {object: true, getDescription: true, translate: 'CHRISPREMADES.Macros.DreadLord.ShadowAttack', identifier: 'dreadLordShadowAttack'});
    if (!featureData) {
    ...
    let effect = await effectUtils.createEffect(workflow.actor, effectData, {identifier: 'dreadLord', vae: [{type: 'use', name: featureData.name, identifier: 'dreadLordShadowAttack'}]});
    if (!effect) return;
...
export let dreadLordShadowAttack = {
